...
class Bert_BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, tag_to_ix, embedding_dim=768, hidden_dim=256):
        ...
        self.tag_to_ix = tag_to_ix#输入crf
        self.tagset_size = len(tag_to_ix)
        self.hidden_dim = hidden_dim#Bilstm
        self.embedding_dim = embedding_dim #embedding
        ...
        self.linear = nn.Linear(hidden_dim, self.tagset_size)#全连接层
    ...
    #这个函数是进行预测的
    def predict_tags(self, input_ids, mask):
        with torch.no_grad():
            emissions = self._get_features(input_ids)
